...
export class UpdateTimeClockUseCase implements UseCase<UpdateTimeClockUseCaseInput, TimeClock> {
  ...
  async exec (input: UpdateTimeClockUseCaseInput):  Promise<TimeClock>{
    ...
    if(!this.verifyIfStartAndEndAreInSameDay(timeClock.clock_time_by_pk.start, input.endTime)) {
      throw Error();
    }

    if(!this.verifyIfEndIsGreaterThanStart(timeClock.clock_time_by_pk.start, input.endTime)) {
      throw Error();
    }

    if(!this.verifyIfWorkJourneyHasMoreThanOneMinute(timeClock.clock_time_by_pk.start, input.endTime)) {
      throw Error();
    }

    const { data } = await hasuraClient.mutate({
      mutation: UpdateTimeClockMutation,
      variables: { id: input.id, end: input.endTime},
    });

    return data?.update_clock_time_by_pk
  }
  
  private verifyIfStartAndEndAreInSameDay (startTime: string, endTime: string): boolean {
